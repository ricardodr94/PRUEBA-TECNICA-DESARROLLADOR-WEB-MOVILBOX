//----------------------------metodo para los datos de cada usuario disponible-------------------------------------

function getUserData() {

    let urlUsers = "http://test.movilbox.co:888/test_mbox/test.php?metodo=usuarios";

    const xhttp = new XMLHttpRequest();
    xhttp.open('GET', urlUsers, true);
    xhttp.send();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let item of userData) {
                ...
                let container_item1 = document.getElementById("menu_user" + number);
                container_item1.innerHTML = "";
                container_item1.innerHTML += `<li> ${ item.nombre }</li>`;
                container_item1.innerHTML += `<li> ${ item.perfil }</li>`;
                container_item1.innerHTML += `<li> Días Planificados: ${ item.dias_plani }</li>`;
                container_item1.innerHTML += `<li style="border-bottom:1px solid #c0c0c0; ">Cant Tiendas Planificados: ${ item.tiendas_plani }</li>`;
                number++;
...
//----------------------metodo para filtar usuarios en el buscador------------------------------------------------------

function searchUsers() {
    let filterUser = document.getElementById("filterUser").value;
    console.log(filterUser);
    let textFilter = filterUser.toLowerCase();
    let urlUsers = "http://test.movilbox.co:888/test_mbox/test.php?metodo=usuarios";
    const xhttp = new XMLHttpRequest();
    xhttp.open('GET', urlUsers, true);
    xhttp.send();
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (let dataUser of userData) {

                let container_item1 = document.getElementById("menu_user" + number);
                container_item1.innerHTML = "";
                let userName = dataUser.nombre.toLowerCase();

                if (userName.indexOf(textFilter) !== -1) {

                    container_item1.innerHTML += `<li> ${ dataUser.nombre }</li>`;
                    container_item1.innerHTML += `<li> ${ dataUser.perfil }</li>`;
                    container_item1.innerHTML += `<li> Días Planificados: ${ dataUser.dias_plani }</li>`;
                    container_item1.innerHTML += `<li style="border-bottom:1px solid #c0c0c0; ">Cant Tiendas Planificados: ${ dataUser.tiendas_plani }</li>`;
...
function showMenu() {
    ...
    let container_menu = document.getElementById("container_menu_contextual");
...
//----------------------------------arrastrar usuario a calendario con jquery ----------------------------------------